...
$pd_id=$_POST["pd_id"];
$sub=$_POST["sub"];
$pc_id_1=$_POST["pc_id_1"];
$rb_bidcost=$_POST["rb_bidcost"];
$pc_id=$_POST["pc_id"];
$data=DataBase::SelectData("SELECT `rb_bidamount` as lowest_unique,`rb_id`, `pd_id`, `ur_id` 
FROM `product_reversebidding` 
where  pd_id=$pd_id  and rb_status='WINNER' order by rb_bidamount LIMIT 1");
$lowestbid=mysqli_fetch_array($data);

DataBase::ExecuteQuery("UPDATE `user` 
SET `ur_balance`=ur_balance-$rb_bidcost where ur_id=".$_COOKIE['lg_urid']);

DataBase::ExecuteQuery("INSERT INTO `userwallet`(`ur_id`, `uw_amount`, `uw_status`, `uw_purpose`, 
`uw_date`) VALUES (".$_COOKIE['lg_urid'].",$rb_bidcost,'withdraw','bidding',CURDATE())");
...
header("Location:products.php?pc_id=$pc_id_1&sub=$sub");
